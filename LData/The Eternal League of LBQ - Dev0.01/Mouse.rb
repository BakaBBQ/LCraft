...
module FSL
  module Mouse_System
    TrueMouse = false               # 使用系统真实鼠标(否则用RM精灵代替)
    CursorFile = "fox_cursor"       # 光标文件名(最好包含后缀)
...
module Mouse
  ...
  def update    
    ...
    @move = MouseMove.call
    @wheel = 0
    @wheel = MouseWheel.call if $mouse_update_wheel
...
#~     puts "getting updated"
  ...
  #
  def trigger?(x)
    return Input.trigger?([:RIGHT,:LEFT, :MIDDLE].index(x))
  end
  
  
  def x
    return get_pos[0] if get_pos
  end
  
  def y
    return get_pos[1] if get_pos
  end
  ...
  #--------------------------------------------------------------------------
  def click?(key = 7)
    return true if down?(key) or dbl_clk?(key)
    return false
  end
  #--------------------------------------------------------------------------
  # ## 双击?
  ...
  unless const_defined? :MouseStart
    begin
      if TrueMouse
        MouseStart = CallBackStart.call(1)
        MouseSetCursor.call(check_path(CursorFile))
      else
        MouseStart = CallBackStart.call(0)
        update_cursor
      end
    rescue Errno::ENOENT
      print("鼠标系统初始化失败..")
